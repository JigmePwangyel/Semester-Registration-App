...
Future<String> getUserName(String userId) async {
  final apiUrl =
      'http://192.168.214.159:3000/user/$userId'; // Replace with your API endpoint
  try {
...
Future<bool> isRegistered(String userId) async {
  final apiUrl = 'http://192.168.214.159:3000/user/$userId/IsRegistered';
  try {
    final response = await http.get(Uri.parse(apiUrl));
...
Future<String> getRegistrationDetail(String userId) async {
  final apiUrl =
      'http://192.168.214.159:3000/registration/$userId/RegistrationDetail'; // Replace with your API endpoint
  try {
    final response = await http.get(Uri.parse(apiUrl));
